...
public class CPU8Bit
{
    ...
    private void ExecuteInstruction(byte opcode)
    {
        switch (opcode)
        {
            ...
            case 0x1A: // LDDA #imm - OBSOLETE: Load DA with 8-bit immediate value (kept for compatibility)
                _regDA = Memory.ReadByte(PC++);
                SR.UpdateZeroFlag(_regDA);
                break;

            case 0x1B: // LDDB #imm - OBSOLETE: Load DB with 8-bit immediate value (kept for compatibility)
                _regDB = Memory.ReadByte(PC++);
                SR.UpdateZeroFlag(_regDB);
                break;
